<x-layouts.auth>
    <x-slot name="pageTitle">Edit Spin History</x-slot>

    <x-auth.card card-header="Edit Spin History" header-button="true">
        ...
        <x-auth.form form-action="{{ route('spinhistories.update', $data->id) }}">
            @method('PUT')
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('spin_result_type') is-invalid @enderror" id="spin_result_type" name="spin_result_type" required>
                        ...
                        @foreach($resultTypes as $key => $label)
                            <option value="{{ $key }}" {{ (string)old('spin_result_type', $data->spin_result_type) === (string)$key ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3" id="points_earned_field" style="display: none;">
                    <x-auth.input-field type="number" name="points_earned" id="points_earned"
                        place="Enter points earned" val="{{ old('points_earned', $data->points_earned) }}" label="Points Earned *" min="0" />
                    @error('points_earned')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
                    ...
                    <select class="form-select @error('offer_id') is-invalid @enderror" id="offer_id" name="offer_id">
                        ...
                        @foreach($offers as $offer)
                            <option value="{{ $offer->id }}"
                                ...
                                {{ (string)old('offer_id', $data->offer_id) === (string)$offer->id ? 'selected' : '' }}>
                                {{ $offer->title }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3" id="reward_value_field" style="display: none;">
                    <x-auth.input-field type="number" name="reward_value" id="reward_value" step="0.01"
                        place="Enter reward value" val="{{ old('reward_value', $data->reward_value) }}" label="Reward Value" min="0" />
                    @error('reward_value')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <label for="spin_number" class="form-label">Spin Number</label>
                    <x-auth.input-field type="number" name="spin_number" id="spin_number"
                        place="Enter spin number" val="{{ old('spin_number', $data->spin_number) }}" label="Spin Number" min="1" />
                </div>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" name="is_eligible" id="is_eligible" value="1" 
                            {{ old('is_eligible', $data->is_eligible) ? 'checked' : '' }}>
            ...
            <div class="d-flex justify-content-end">
                <x-auth.input-button btn-class="btn-primary" btn-type="submit" btn-value="Update Spin History" />
            </div>
        </x-auth.form>
    </x-auth.card>
</x-layouts.auth>
