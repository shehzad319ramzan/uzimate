            ...
            <div class="row">
                ...
                <div class="col-md-8 mb-3">
                    <x-auth.input-field type="text" name="title" id="title" required="true" place="Voucher title"
                        val="{{ old('title', $data->title) }}" label="Title" />
                </div>
                <div class="col-md-4 mb-3">
                    <x-auth.input-field type="number" name="points_required" id="points_required" required="true"
                        place="e.g. 200" val="{{ old('points_required', $data->points_required) }}" label="Points required" />
                </div>
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('merchant_id') is-invalid @enderror" name="merchant_id" id="merchant_id" required>
                        ...
                        @foreach ($merchants as $m)
                            <option value="{{ $m->id }}" {{ old('merchant_id', $data->merchant_id) == $m->id ? 'selected' : '' }}>{{ $m->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    <x-auth.select2
                        ...
                        id="offer_ids"
                        :data="$offers"
                        :existing-id="old('offer_ids', $data->offers->pluck('id')->toArray())"
                        placeholder="Select one or more offers"
                        selectclass="select2-offers"
                        multiple
                    />
                    <small class="text-muted">Select offers to link this voucher to.</small>
                </div>
                <div class="col-md-6 mb-3">
                ...
                <div class="col-md-12 mb-3">
                    <x-auth.editor name="terms_and_conditions" label="Terms and conditions" id="terms_and_conditions" :value="old('terms_and_conditions', $data->terms_and_conditions ?? '')" />
                </div>
            </div>
            <div class="row">
